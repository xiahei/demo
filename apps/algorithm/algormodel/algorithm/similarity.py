...
class Sim:
    ...
    def load_stopwords(self,stopwords_file=BASE_DIR+'/stopwords'):
        ...
        with open(stopwords_file,"r",encoding="UTF-8") as fr:
            for line in fr:
                line = line.strip() #turn string to utf-8
                stopwords.add(line)
        return stopwords
    ...
    def search_range(self,pred_label):
        ...
        inputname = BASE_DIR+'/data/'+self.label_dict[int(pred_label)]
        with open(inputname,'r',encoding='UTF-8') as fr:
            for line in fr:
    ...
    def cal_sim(self,train_matrix,test_matrix,train_index):
        ...
        max_value = np.max(sim)
        print ("max_value:",max_value)
        if max_value > 0.4:
            sim_q = train_index[max_index][0]
            sim_a = train_index[max_index][1]
        else:
            sim_q = None
            sim_a = None
        return sim_q,sim_a
